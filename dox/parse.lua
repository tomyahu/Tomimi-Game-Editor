...
--[[!
@module docs.dox.parse
@func blockToTable
@desc Searches through a block (of any kind: module, function, etc.) and splits it based on the 'docs.dox.SpecialChars[1].Intended' special character.
...
@desc Adds a module entry to the 'docs.dox.Modules' table (if it doesn't already exist).
...
@desc Gets what the default value should be for each item in the 'tRet' table found within the 'processBlock()' function. It is worth noting that placing a default value at 'docs.dox.Types[sType].DefaultValue' where 'sType' is the name of the type desired, causes this function to return that value.
...
--[[!
@module docs.dox.parse
@func docs.dox.parse.getBlocks
@desc Retrieves the blocks (such as module and function blocks) from a string of lua code. Used in 'docs.dox.processFile()'.
...
--[[!
@module docs.dox.parse
@func docs.dox.parse.getModuleCount
@desc Counts the number of modules to rpevent processing if none are present. Used in 'docs.dox.processFile()'.
...
--[[!
@module docs.dox
@func docs.dox.parse.importBlockToModules
@desc Pushes a block into the 'docs.dox.Modules' table to await processing. Used in 'docs.dox.processFile()'.
!]]
...
--[[!
@module docs.dox.parse
@func docs.dox.parse.importModuleInfo
...
--[[!
@module docs.dox.parse
@func docs.dox.parse.processBlock
@desc Processes a function block and validates all the lines input in the blocks string. Used in 'docs.dox.processFile()'.
...
local tRet;
	...
	if type(sBlock) == "string" then
		...
		tRet = {
			...
			--escape special characters with something else until the table is constructed.
			--[[for nIndex, tChar in pairs(docs.dox.SpecialChars) do
			sBlock = sBlock:gsub(tChar.Escaped, tChar.Temp);
			end]]
...
--[[!
@module docs.dox.parse
@func docs.dox.parse.sortModules
...
function dox.parse.sortModules()
	...
	local tModuleIndexer = {};
		...
		--store the 'docs.dox.Modules' names in a numerically-indexed table for sorting
