...
function ActionAnimations.makeScene(self, source_entity_view, target_entity_views)
    ...
    table.insert(animation_sequences, self:getSourceAnimationBuilder():getAnimationSequence(source_entity_view, target_entity_views))

    -- Add the target entities views of the action to the animation
    for _, entity_view in pairs(target_entity_views) do
        table.insert(animation_sequences, self:getTargetAnimationBuilder():getAnimationSequence(entity_view, {source_entity_view}))
    end
